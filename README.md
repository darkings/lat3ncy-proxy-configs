...
Quantumult X 配置使用对应 Raw 链接导入。Windows 文件不是节点订阅，而是 Clash Verge Rev 远程扩展配置：保留已有节点订阅，在“订阅”页面新建远程扩展配置，填入上面的 Raw 链接并关联到现有订阅，然后重新激活该订阅。
...
Windows 版用于 Clash Verge Rev 的 Mihomo 内核，作为远程扩展配置叠加在私人节点订阅上。默认使用规则模式、系统代理和 Fake-IP DNS，不开放局域网代理；TUN 默认关闭，以减少与 WSL、Tailscale 和其他虚拟网络的冲突。

配置将 `100.64.0.0/10`、`fd7a:115c:a1e0::/48`、`*.ts.net` 和 Tailscale 控制域名置于直连规则最前，并从 TUN 路由和 Fake-IP 解析中排除。Cats-Team AdRules 作为桌面端广告主规则，每 6 小时自动更新。浏览器页面的元素隐藏仍建议交给浏览器内容拦截扩展处理。
...
Windows 版作为远程扩展配置使用：在 Clash Verge Rev 中刷新该扩展并重新激活节点订阅即可应用新版，不需要删除或重新导入节点订阅。Cats-Team 规则由 Mihomo 按 `interval` 自动更新。
