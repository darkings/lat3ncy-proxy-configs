...
Quantumult X 配置使用对应 Raw 链接导入。Windows YAML 不包含节点，不能当作普通订阅单独激活；它需要作为“订阅扩展配置”叠加到已有节点订阅上。
...
Windows 版用于 Clash Verge Rev 的 Mihomo 内核，以单个远程 YAML 扩展配置叠加在私人节点订阅上。配置使用规则模式和 Fake-IP DNS，不开放局域网代理；默认关闭 TUN，建议开启系统代理，以减少与 WSL、Tailscale 和其他虚拟网络的冲突。

### Windows 配置方法

1. 先在 Clash Verge Rev 中导入、更新并激活自己的节点订阅。
2. 打开上方 Windows Raw 地址，复制页面中的全部 YAML 内容。
3. 右键节点订阅卡片，选择“编辑扩展/脚本”，进入“订阅扩展配置”，粘贴并保存 YAML。
4. 重新激活节点订阅，使用规则模式，开启系统代理并保持 TUN 关闭。

不要把 Windows Raw 地址添加成普通节点订阅，也不需要导入 JavaScript。

单 YAML 完整接管节点订阅的策略组和分流规则，同时继续使用订阅提供的节点。配置将 `100.64.0.0/10`、`fd7a:115c:a1e0::/48`、`*.ts.net`、Tailscale 控制域名、私有域名/IP 和 Windows 联网检测置于规则最前；Cats-Team AdRules 每 6 小时更新，MetaCubeX 私有网络及国内外分流 MRS 每天更新。

Windows 版为 OpenAI、GitHub、Microsoft、OneDrive、Steam、Apple、YouTube、Spotify 和 Telegram 提供独立策略组及 MetaCubeX 规则。Microsoft、Steam 与 Apple 的中国区下载/CDN 子集优先直连，商店、社区和国际服务进入对应策略组；不加入 TikTok、抖音、拼多多等移动 App 专项规则，也不包含 Quantumult X 式 HTTPS 响应重写。浏览器页面的元素隐藏仍建议交给浏览器内容拦截扩展处理。
...
Windows 版更新时，重新打开 Raw 地址并复制新版 YAML，覆盖“订阅扩展配置”中的旧内容，保存后重新激活节点订阅；不需要删除或重新导入节点订阅。Cats-Team 与 MetaCubeX 规则由 Mihomo 按 `interval` 自动更新。
