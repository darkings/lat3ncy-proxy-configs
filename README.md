...
这个仓库保存我当前自用的 Quantumult X 手机版、Quantumult X macOS 版、Loon iOS 版、Loon macOS 版和 Sparkle Windows 版配置。配置按自己的设备、常用应用和 Tailscale 网络维护，不作为适合所有环境的通用模板。公开内容不包含私人节点订阅、MITM 证书、密码或 Token。
...
Quantumult X 和 Loon 配置使用对应 Raw 链接导入。Loon 导入后需在本地添加节点订阅；Windows YAML 不包含节点，不能作为普通订阅单独激活，它需要在 Sparkle 中作为远程 YAML 覆写绑定到节点订阅或内置 Sub-Store 的 ClashMeta 输出。
...
两份 Loon 配置基于 iKeLee 的 Auto Select 模板定制并保留原作者和授权说明。Loon 配置需要在本地添加节点订阅，MITM 证书也只在 Loon 本地生成和安装；公开文件中的节点、远程订阅和证书字段保持为空。

两端都启用 IPv6 和系统 DNS，并让 Tailscale 的 100.64.0.0/10、fd7a:115c:a1e0::/48、*.ts.net、*.tailscale.com 优先直连。iOS 继续绕过代理和 TUN；macOS 还将 100.100.100.100/32 加入 `skip-proxy`，但不放入 `bypass-tun`，避免与 Tailscale 发生路由冲突。香港、台湾、日本、新加坡、美国是五个固定地区自动测速组；只有订阅节点名称匹配对应地区正则时，组内才会出现节点。

顶层策略顺序为 Proxy、各应用组、Auto 和五个地区组。Proxy 默认使用 Auto，也可切换地区、DIRECT 或单个订阅节点；应用组可选择 Proxy、DIRECT 和地区组。iOS 版包含 TikTok、BoxJs、Sub-Store、Script-Hub 等移动端工具，TestFlight 保持关闭；macOS 版用 Steam 替代 TikTok，只保留广告平台、DNS 泄漏防护、QuickSearch、节点检测和 Sub-Store。
...
Quantumult X 和 Loon 主配置不会实时自动更新。仓库发布新版后，需要使用对应 Raw 链接刷新或重新导入；Quantumult X 引用的远程规则和脚本按照各自的 `update-interval` 自动检查上游更新，Loon 的远程规则和插件则由客户端资源更新机制刷新。
